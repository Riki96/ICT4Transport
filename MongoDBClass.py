...
class MyMongoDB:
			...
			for a in collections:
				...
				for index, row in dataframe.iterrows():
					...
					if dataframe.loc[index,'duration'] > 1:
						...
						for i in range (dataframe.loc[index,'duration']-1):
							try:
	...
	def statistics(self, start_date, end_date, start_ny, end_ny, cities, days=[d for d in range(1,32)]):
		"""
		...
		for c in cities:
			...
			plt.savefig('Plots/City_of_{}_-_Bookings_Statistics.png'.format(c), dpi=300)
			...
			plt.savefig('Plots/City_of_{}_-_Parkings_Statistics.png'.format(c), dpi=300)			
	...
	def closest_to(self, O, D, lat_min=45.01089, long_min=7.60679):
		z = np.linspace(0,0.1,15)
